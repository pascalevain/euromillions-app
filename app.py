...
# Configuration de la page (doit être juste après l'import de streamlit)
...
fichier = st.file_uploader("📂 Chargez ici le fichier bitmap (.csv)", type="csv")
if fichier:
    historique = pd.read_csv(fichier)
    st.success(f"Historique chargé : {len(historique)} tirages.")
